...
export async function getAverageFuelOrBatteryLevel(fleet_id) {
    try {
        const averageFuelOrBatteryLevel = await db.execute(`SELECT AVG(temporaryTable.fuel_or_battery_level) as averageFuelOrBatteryLevel FROM (SELECT DISTINCT T.vin, T.fuel_or_battery_level FROM TELEMETRY T JOIN VEHICLES V ON T.vin = V.vin WHERE V.fleet_id = '${fleet_id}' AND T.TIMESTAMP = (SELECT MAX(TIMESTAMP) FROM TELEMETRY WHERE VIN = T.VIN)) AS temporaryTable;`)
